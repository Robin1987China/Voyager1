...
public class TransformServerFactory {
    ...
    private static TransformServer doCreate() {
        final TransformServer engine = java.util.ServiceLoader.load(TransformServer.class).findFirst().orElse(null);
        if (null != engine) {
            return engine;
        }

        throw new RuntimeException("No voyager1 agent transform jar found ! Please add one of it to your project !");
    }
}

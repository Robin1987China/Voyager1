...
public abstract class AbstractProjectCommander implements ProjectCommander {
     ...
     * 进程Id 获取端口号
     */
    public static final Map<Integer, CacheObject<String>> PID_PORT = new java.util.concurrent.ConcurrentHashMap<>();
...
//        if ((nodeProjectInfoModel.getJdkId() == null || nodeProjectInfoModel.getJdkId().isEmpty())) {
//            return w ? "javaw" : "java";
//        }
//
//        if (item == null) {
...
//    }
    ...
    private Tuple stopBefore(NodeProjectInfoModel nodeProjectInfoModel, NodeProjectInfoModel originalModel) {
        ...
        return new Tuple((beforeStop == null || beforeStop.isEmpty() ? "" : beforeStop), result);
    }
    ...
    public void asyncWebHooks(NodeProjectInfoModel nodeProjectInfoModel,
        ...
        // 判断文件变动
        if (java.util.Objects.equals(type, "fileChange")) {
            RunMode runMode = originalModel.getRunMode();
            if (runMode == RunMode.Dsl) {
    ...
    private String webHooks(String token, NodeProjectInfoModel nodeProjectInfoModel, String type, Object... other) {
        ...
        map.put("type", type);
        map.put("VOYAGER1_WEBHOOK_EVENT", DefaultWebhookPluginImpl.WebhookEvent.PROJECT);
        ...
        try {
            ...
            return ConvertUtil.toStr(execute, "");
        } catch (Exception e) {
            throw Lombok.sneakyThrow(e);
        }
    }
    ...
    public String backLog(NodeProjectInfoModel nodeProjectInfoModel, NodeProjectInfoModel originalModel) {
        ...
        if (openLogBack) {
            ...
            String pathId = DateTime.now().toString("yyyyMMddHHmmss") + ".log";
    ...
    protected CommandOpResult status(NodeProjectInfoModel nodeProjectInfoModel, NodeProjectInfoModel originalModel) {
        ...
        if (runMode == RunMode.Dsl) {
            ...
            try {
                ...
                status = new java.util.ArrayList<>(java.util.Arrays.asList(argument2Exception.getMessage()));
            }
            ...
            return Optional.ofNullable(status)
                ...
                // 此流程特意处理 系统日志标准格式 String.format("%s [%s] - %s", DateUtil.now(), this.action, line);
    ...
    protected String status(String tag) {
        ...
        if (java.util.Objects.equals(AbstractProjectCommander.STOP_TAG, jpsStatus)) {
            // 通过系统命令查询
    ...
    private String getJpsStatus(String tag) {
        ...
        return String.format("%s:%s", AbstractProjectCommander.RUNNING_TAG, pid);
    }
...
//---------------------------------------------------- 基本操作----end
    ...
    public String getMainPort(Integer pid) {
        ...
        for (NetstatModel model : list) {
            ...
            // 取最小的端口号
            int minPort = ConvertUtil.toInt(portStr, Integer.MAX_VALUE);
        ...
        String allPort = ports.stream().map(String::valueOf).collect(java.util.stream.Collectors.joining(","));
        // 缓存
    ...
    private String getPortFormLocalIp(String local) {
        ...
        List<String> ipPort = StrSplitter.splitTrim(local, ":", true);
        if (ipPort.isEmpty()) {
            return null;
        }
    ...
    protected boolean loopCheckRun(NodeProjectInfoModel nodeProjectInfoModel, NodeProjectInfoModel originalModel, int waitTime, boolean status) {
        ...
        do {
            ...
            try {
                Thread.sleep(statusDetectionInterval);
    ...
    protected String getClassPathLib(NodeProjectInfoModel nodeProjectInfoModel, String lib) {
        ...
        if (runMode == RunMode.ClassPath) {
            ...
            String[] split = javaExtDirsCp.split(java.util.regex.Pattern.quote(":"));

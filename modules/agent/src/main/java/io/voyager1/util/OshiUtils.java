...
import io.voyager1.util.CpuInfo;
import io.voyager1.util.OshiUtil;
...
public class OshiUtils {
    ...
    public static JSONObject getSystemInfo() {
        ...
        jsonObject.put("osVersion", String.format("%s %s %s", manufacturer, family, versionStr));
        ...
        jsonObject.put("hardwareVersion", String.format("%s %s", computerSystem.getManufacturer(), computerSystem.getModel()));
    ...
    private static NetIoInfo getNetInfo(MonitorConfig.NetworkConfig networkConfig) {
        ...
        StringBuilder ifsNames = new StringBuilder("");
        if (listBegin != null) {
            listBegin = listBegin.stream()
                .filter(networkIF -> (statExcludeNames == null || statExcludeNames.isEmpty()) || !isMatch(statExcludeNames, networkIF.getName()))
                .filter(networkIF -> (statContainsOnlyNames == null || statContainsOnlyNames.isEmpty()) || isMatch(statContainsOnlyNames, networkIF.getName()))
                .collect(Collectors.toList());
    ...
    public static List<JSONObject> getProcesses(String name, int count) {
        ...
        return operatingSystem.getProcesses(
                osProcess -> (osProcess.getName() != null && osProcess.getName().equalsIgnoreCase(name)),

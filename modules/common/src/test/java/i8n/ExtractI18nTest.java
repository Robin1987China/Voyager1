...
/**
 * <h1>Voyager1 后端 i18n 工具自动化流程</h1>
 * <p>
 * <ol>
 *     <li>提取代码中的所有关联的中文</li>
...
public class ExtractI18nTest {
    ...
    public final static String[] Voyager1Annotation = {
    ...
    public void extractJavaZh_First1() throws Exception {
        ...
        for (Map.Entry<String, Long> entry : collect.entrySet()) {
            ...
            Assertions.assertEquals(1L, value, "[" + entry.getKey() + "]出现去重空格后重复");
        }
        // 语意化中文存储为 key（排序）
        ...
        int pageSize = 50;
        int total = (wordsSet == null ? 0 : wordsSet.size());
        ...
        for (int i = PageUtil.getFirstPageNo(); i <= page; i++) {
            ...
            Collection<String> sub = new java.util.ArrayList<>(wordsSetSort).subList(start, end);
            Collection<String> subAfter = filterExists(sub, cacheWords, allResult);
            if ((subAfter != null && !subAfter.isEmpty())) {
                while (true) out:{
    ...
    public void generateZhPropertiesAndReplace_Second2() throws Exception {
        ...
        for (Object keyObj : new java.util.ArrayList<>(java.util.Arrays.asList(zhProperties.keySet()))) {
        ...
        Assertions.assertTrue((notUseKeys == null || notUseKeys.isEmpty()), "存在未使用的 key:" + notUseKeys);
    }
    ...
    public void syncZhToEnProperties_Third3() throws Exception {
        // 加载中文配置
        Properties zhProperties = new Properties();
        try (BufferedReader inputStream = FileUtil.getReader(zhPropertiesFile)) {
            zhProperties.load(inputStream);
        }
    ...
    private void syncZhToProperties(String language, String volcLang) throws Exception {
        ...
        // 接口限制、不能超过 16
        int pageSize = 16;
        int total = (keySets == null ? 0 : keySets.size());
        int page = PageUtil.totalPage(total, pageSize);
        //
        ...
        for (int i = PageUtil.getFirstPageNo(); i <= page; i++) {
            ...
            List<Object> keys = new java.util.ArrayList<>(keySets).subList(start, end);
    ...
    private String checkExists(JSONObject jsonObject, String value) {
        for (Map.Entry<String, Object> entry : jsonObject.entrySet()) {
            ...
            if (java.util.Objects.equals(entryValue, value)) {
                // 值相同
                String key = entry.getKey();
                List<String> split = java.util.Arrays.asList(key.split(java.util.regex.Pattern.quote(".")));
                String last = (split == null || split.isEmpty() ? null : split.get(split.size() - 1));
                String md5 = DigestUtil.md5(value);
                if ((md5 != null && md5.startsWith(last))) {
                    // 可以的后缀也等于值的 md5 前缀
                    return key;
                }
    ...
    private String buildKey(String key, String value, JSONObject jsonObject) {
        ...
        while (true) {
            String md5 = DigestUtil.md5(value);
            Assertions.assertTrue(md5.length() >= md5IdLen, "截取中文 md5 key 超范围：" + value);
            md5 = md5.substring(0, md5IdLen);
            String newKey = String.format("i18n.%s.%s", StrUtil.toUnderlineCase(key), md5);
    ...
    private String findOriginal(Collection<String> list, String value) {
        for (String s : list) {
            if (java.util.Objects.equals(s, value) || java.util.Objects.equals((s == null ? null : s.trim()), value)) {
                // 需要返回原始值，不能返回翻译后的值
                return s;
            }
        }
    ...
    public static void walkFile(File file, Consumer<File> consumer) {
        FileUtil.walkFiles(file, file1 -> {
            ...
            if (java.util.Objects.equals("java", FileUtil.extName(file1))) {
            ...
            while ((line = reader.readLine()) != null) {
                if (canIgnore(line)) {
                    ...
                    while (matcher.find()) {
                        ...
                        if (StrUtil.containsAny(line, Voyager1Annotation)) {
    ...
    public static boolean canIgnore(String line) throws ClassNotFoundException {
        ...
        if (StrUtil.startWithAny(trimLin, Voyager1Annotation)) {
            // voyager1 特有注解
            return false;
        }

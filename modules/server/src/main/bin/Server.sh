...
# description: Auto-starts voyager1 server
...
PID_TAG="VOYAGER1_SERVER_APPLICATION"
...
function start() {
  ...
  echo "Voyager1 server starting:$pid"
  pid=$(getPid)
  if [ "$pid" == "" ]; then
...
function stop() {
  ...
  if [ "$pid" != "" ]; then
    echo -n "voyager1 server ( pid $pid) is running"
    echo
    ...
    echo "voyager1 server is stopped"
  fi
  eval "$(rm -f "$pidfile")"
}
...
function status() {
  ...
  #echo "$pid"
  if [ "$pid" != "" ]; then
    echo "voyager1 server running:$pid"
  else
    echo "voyager1 server is stopped"
  fi
}

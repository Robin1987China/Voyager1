...
public class UserListController extends BaseServerController {
    ...
    private UserModel parseUser(boolean create) {
        ...
        if (email) {
            ...
            Assert.state(length <= Const.ID_MAX_LEN && length >= UserModel.USER_NAME_MIN_LEN, String.format("登录名如果为邮箱格式, 长度必须 {}-{}", UserModel.USER_NAME_MIN_LEN, Const.ID_MAX_LEN));
        } else {
            String checkId = id.replace("-", "_");
            Validator.validateGeneral(checkId, UserModel.USER_NAME_MIN_LEN, Const.ID_MAX_LEN, String.format("登录名格式不正确（英文字母 、数字和下划线）, 并且长度必须 {}-{}", UserModel.USER_NAME_MIN_LEN, Const.ID_MAX_LEN));
        }

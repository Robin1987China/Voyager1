...
public class MachineDockerController extends BaseGroupNameController {
    ...
    public ApiResult<PageResultDto<MachineDockerModel>> listJson(HttpServletRequest request) {
    ...
    public ApiResult<Object> edit(String id) throws Exception {
    ...
    private MachineDockerModel takeOverModel() throws Exception {
        ...
        boolean enableSsh = ConvertUtil.toBool(enableSshStr, false);
        String machineSshId = getParameter("machineSshId");
        boolean sshUseSudo = ConvertUtil.toBool(getParameter("sshUseSudo"), false);
        if (enableSsh) {
            MachineSshModel model = machineSshServer.getByKey(machineSshId);
            host = "ssh://" + model.getHost() + ":" + (model.getPort() == null ? 22 : model.getPort());
        }
        boolean tlsVerify = ConvertUtil.toBool(tlsVerifyStr, false);
        //
        if (tlsVerify) {
            ...
            certInfo = (certInfo == null || certInfo.isEmpty() ? "" : certInfo);
        }
    ...
    @Feature(method = MethodFeature.EDIT)
    public ApiResult<String> tryLocalDocker(HttpServletRequest request) {
        try {
            IPlugin plugin = PluginFactory.getPlugin(DockerInfoService.DOCKER_CHECK_PLUGIN_NAME);
            ...
            boolean exists = machineDockerServer.exists(entity);
            if (exists) {
                return new ApiResult<>(405, "已经存在本地 docker 信息啦，不要重复添加：" + dockerHost);
            }
            ...
            return new ApiResult<>(200, "自动探测到本地 docker 并且自动添加：" + dockerHost);
        } catch (Throwable e) {
            log.error("探测本地 docker 异常", e);
            return new ApiResult<>(500, "探测本地 docker 异常：" + e.getMessage());
        }
    }

    @GetMapping(value = "del", produces = MediaType.APPLICATION_JSON_VALUE)
    @Feature(method = MethodFeature.DEL)
    public ApiResult<Object> del(@ValidatorItem String id) throws Exception {
        //
        {
            ...
            Assert.state(count <= 0, String.format("当前 docker 还关联%s个 工作空间 docker 不能直接删除（需要提前解绑或者删除关联数据后才能删除）", count));
        }
        MachineDockerModel infoModel = machineDockerServer.getByKey(id);
        Optional.ofNullable(infoModel).ifPresent(machineDockerModel -> {
            if ((machineDockerModel.getSwarmId() != null && !machineDockerModel.getSwarmId().isEmpty())) {
                // 判断集群
                ...
                Assert.state(count <= 0, String.format("当前 docker 还关联%s个 工作空间 docker 集群，不能直接删除（需要提前解绑或者删除关联数据后才能删除）", count));
            }
        });
    ...
    @Feature(method = MethodFeature.EDIT)
    public ApiResult<String> distribute(@ValidatorItem String ids, @ValidatorItem String workspaceId, String type) {
        ...
        for (String id : list) {
            ...
            if (java.util.Objects.equals(type, "docker")) {
                DockerInfoModel dockerInfoModel = new DockerInfoModel();
                dockerInfoModel.setMachineDockerId(id);
            ...
            } else if (java.util.Objects.equals(type, "swarm")) {
                Assert.hasText(machineDockerModel.getSwarmId(), () -> String.format("当前 docker %s 不在集群中", machineDockerModel.getName()));
                DockerSwarmInfoMode dockerInfoModel = new DockerSwarmInfoMode();
                dockerInfoModel.setSwarmId(machineDockerModel.getSwarmId());
                dockerInfoModel.setWorkspaceId(workspaceId);
                //
    ...
    @Feature(method = MethodFeature.LIST)
    public ApiResult<JSONObject> listWorkspaceSsh(@ValidatorItem String id) {
        MachineDockerModel machineDockerModel = machineDockerServer.getByKey(id);
        Assert.notNull(machineDockerModel, "没有对应的 docker");
    ...
    public ApiResult<String> importTls(MultipartFile file) throws Exception {
        Assert.notNull(file, "没有上传文件");
        // 保存路径
        File tempPath = FileUtil.file(serverConfig.getUserTempPath(), "docker", java.util.UUID.randomUUID().toString().replace("-", ""));
        try {

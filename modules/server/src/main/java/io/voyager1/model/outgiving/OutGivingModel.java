        ...
        if (outGivingNodeProjects != null) {
            ...
            for (int i = 0; i < outGivingNodeProjects.size(); i++) {
                ...
                if (outGivingNodeProject.getSortValue() != null) {
                    outGivingNodeProject.setSortValue((outGivingNodeProject.getSortValue() != null ? outGivingNodeProject.getSortValue() : i));
                }
            }
            List<String> list = io.voyager1.util.ConvertUtil.splitTrim(select, ",");
            outGivingNodeProjects = outGivingNodeProjects.stream()
                .filter(nodeProject -> {
                    ...
                    return list.stream()
                        .anyMatch(s -> java.util.Objects.equals(s, String.format("%s@%s", nodeProject.getProjectId(), nodeProject.getNodeId())));
                })

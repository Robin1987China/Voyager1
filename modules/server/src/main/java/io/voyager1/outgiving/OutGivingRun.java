...
public class OutGivingRun {
    ...
    private static final Map<String, Map<String, String>> LOG_CACHE_MAP = new java.util.concurrent.ConcurrentHashMap<>();
...
//     * @param clearFile       是否清空文件
    ...
    public static String getLogId(String outId, OutGivingNodeProject nodeProject) {
        ...
        String dataId = String.format("%s_%s", nodeProject.getNodeId(), nodeProject.getProjectId());
    ...
    public Future<OutGivingModel.Status> startRun(String select) {
        OutGivingServer outGivingServer = SpringContextHolder.getBean(OutGivingServer.class);
        ...
        if (afterOpt == AfterOpt.Order_Restart || afterOpt == AfterOpt.Order_Must_Restart) {
            ...
            syncFinisher.addWorker(() -> {
                try {
                    // 截取睡眠时间
                    int sleepTime = (item.getIntervalTime() != null ? item.getIntervalTime() : 10);
                    //
                    int nowIndex;
                    ...
                    // 取消后面的分发
                    List<OutGivingNodeProject> cancelList = outGivingNodeProjects.subList(nowIndex + 1, outGivingNodeProjects.size());
        ...
        return () -> {
            ...
            try {
                ...
                if (statusList.size() != projectSize) {
                    ...
                    msg = String.format("完成的个数不足 %s/%s", statusList.size(), projectSize);
                } else {
                    ...
                    if (successCount == projectSize) {
                        ...
                        status = OutGivingModel.Status.FAIL;
                        msg = String.format("完成并成功的个数不足 %s/%s", successCount, projectSize);
                    }
                }
    ...
    private void allPrepare(String userId, OutGivingModel outGivingModel, List<OutGivingNodeProject> outGivingNodeProjects) {
        ...
        List<OutGivingLog> outGivingLogs = outGivingNodeProjects.stream()
            .map(outGivingNodeProject -> {
                ...
                // 限制最大长度
                outGivingLog.setModeData((modeData == null ? null : (modeData.length() <= 400 ? modeData : modeData.substring(0, 400))));
                return outGivingLog;
            })
        ...
        //
        Map<String, String> logIdMap = CollStreamUtil.toMap(outGivingLogs, outGivingLog -> String.format("%s_%s", outGivingLog.getNodeId(), outGivingLog.getProjectId()), BaseIdModel::getId);
            ...
            .ifPresent(webhook ->
                I18nThreadUtil.execute(() -> {
                    ...
                    try {
                        ...
                        map.put("VOYAGER1_WEBHOOK_EVENT", DefaultWebhookPluginImpl.WebhookEvent.DISTRIBUTE);
     ...
     * @return json
     */
    public static ApiResult<String> fileUpload(File file, String fileName, String levelName, String projectId,
                                                 boolean unzip,
                                                 AfterOpt afterOpt,
                                                 NodeModel nodeModel,
                                                 boolean clearOld,
     ...
     * @return json
     */
    public static ApiResult<String> fileUpload(File file,
                                                 String fileName,
                                                 String levelName,
                                                 String projectId,
                                                 boolean unzip,

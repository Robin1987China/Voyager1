...
public class DockerBuild implements AutoCloseable {
    ...
    public int build() {
        ...
        // 生成临时目录
        tempDir = FileUtil.file(tempDir, "docker-temp", java.util.UUID.randomUUID().toString().replace("-", ""));
        ...
        String buildId = env.get("VOYAGER1_BUILD_ID");
        String runsOn = (String) parameter.get("runsOn");

        String image = String.format("%s/runs_%s", System.getenv().getOrDefault("VOYAGER1_IMAGE_REPO", "voyager1"), runsOn);
    ...
    private String replaceEnv(String value) {
        ...
        for (Map.Entry<?, ?> envEntry : env.entrySet()) {
            ...
            value = value.replace("${" + envEntry.getKey() + "}", envValue);
        }
        return value;
    }
        ...
        if (hostConfigMap != null) {
            ...
            for (Map.Entry<String, String> entry : entrySet) {
                ...
                if (field != null) {
                    ...
                    Object typeValue = ConvertUtil.convert(type, value);
                    if (typeValue != null) {
    ...
    private void copyArchiveToContainerCmd(DockerClient dockerClient, String containerId, List<String> copy, LogRecorder logRecorder) {
        ...
        for (String s : copy) {
            // C:\Users\wesley\voyager1\server\data\build\5c631117d4834dd4833c04dc1e6e635c\source:/home/voyager1/:true
            String resource;
            String remotePath;
            ...
            if (SystemUtil.getOsInfo().isWindows() && StrUtil.length(split.get(0)) == 1) {
                // 第一位是盘符
                resource = split.get(0) + ":" + split.get(1);
                remotePath = split.get(2);
                dirChildrenOnly = ConvertUtil.toBool((3 < split.size() ? split.get(3) : null), true);
            } else {
                resource = split.get(0);
                remotePath = split.get(1);
                dirChildrenOnly = ConvertUtil.toBool((2 < split.size() ? split.get(2) : null), true);
            }
    ...
    private String otherScript(Map<String, Object> step, String name) {
        ...
        String path = String.format("/opt/voyager1_%s_%s",name,version);
        StringBuilder script = new StringBuilder("# " + name + "Script\n");
        Map<String, String> map = new HashMap<>();
        map.put("VOYAGER1_PLUGIN_PATH", path);
    ...
    private String mavenScript(Map<String, Object> step) {
        String version = String.valueOf(step.get("version"));
        String path = String.format("/opt/voyager1_maven_%s", version);
    ...
    private String goScript(Map<String, Object> step) {
        String version = String.valueOf(step.get("version"));
        String path = String.format("/opt/voyager1_go_%s", version);
    ...
    private String gradleScript(Map<String, Object> step) {
        String version = String.valueOf(step.get("version"));
        String path = String.format("/opt/voyager1_gradle_%s", version);
    ...
    private String javaScript(Map<String, Object> step) {
        String version = String.valueOf(step.get("version"));
        String path = String.format("/opt/voyager1_java_%s", version);
    ...
    private String python3Script(Map<String, Object> step) {
        String version = String.valueOf(step.get("version"));
        String path = String.format("/opt/voyager1_python3_%s", version);
    ...
    private String nodeScript(Map<String, Object> step) {
        String version = String.valueOf(step.get("version"));
        String path = String.format("/opt/voyager1_node_%s", version);
    ...
    private String buildCacheName(Map<String, Object> step, String buildId) {
        ...
        return global ? String.format("voyager1_cache_%s", md5) : String.format("voyager1_cache_%s_%s", buildId, md5);
    }
    ...
    private List<Mount> cachePluginCheck(DockerClient dockerClient, List<Map<String, Object>> steps, String buildId) {
        return steps.stream()
            .filter(map -> {
                ...
                return java.util.Objects.equals("cache", uses);
            })
            .map(objMap -> {
                ...
                try {
                    ...
                    HashMap<String, String> labels = new HashMap<>(java.util.Map.of("voyager1_build_" + buildId, buildId));
                    String path = (String) objMap.get("path");
                    String type = StrUtil.toString(objMap.get("type"));
                    labels.put("voyager1_build_path", path);
                    labels.put("voyager1_build_cache_type", type);
    ...
    public static boolean hasDependPlugin(Map<String, Object> parameter) {
        ...
        String version = Optional.ofNullable(parameter.get("version"))
            ...
            .map(s -> "_" + s)
            .orElse("");
        String name = String.format("voyager1_%s%s", pluginName, version);
    ...
    private Mount dependPluginCheck(DockerClient dockerClient, String image, Map<String, Object> usesMap, String buildId, File tempDir, LogRecorder logRecorder) {
        ...
        String name = String.format("voyager1_%s%s", pluginName, version2);
        if (!DockerBuild.hasDependPlugin(dockerClient, name)) {
            HashMap<String, String> labels = new HashMap<>(java.util.Map.of("voyager1_build_" + buildId, buildId));
            labels.put("voyager1_build_cache", "true");
            //
            dockerClient.createVolumeCmd()
                .withName(name)
                .withLabels(labels)
            ...
            HostConfig hostConfig = new HostConfig()
                .withAutoRemove(true).withMounts(new java.util.ArrayList<>(java.util.Arrays.asList(mount)));

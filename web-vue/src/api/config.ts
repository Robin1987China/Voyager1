...
const voyager1Window_ = window as unknown as GlobalWindow
const apiTimeout: number = Number(
  voyager1Window_.apiTimeout === '<apiTimeout>' ? delTimeout : voyager1Window_.apiTimeout
)
// debug routerBase
const routerBase: string = voyager1Window_.routerBase === '<routerBase>' ? '' : voyager1Window_.routerBase

const pro: boolean = process.env.NODE_ENV === 'production'

const baseURL = import.meta.env.VOYAGER1_BASE_API_URL

const parseTransportEncryption = () => {
  if (voyager1Window_.transportEncryption === '<transportEncryption>') {
    return 'NONE'
  }
  return voyager1Window_.transportEncryption || 'NONE'
}
...
async function request<T = any>(arg: string | AxiosRequestConfig, config?: AxiosRequestConfig): Promise<IResponse<T>> {
  ...
  if (data.code === 999) {
    ...
    window.location.href = voyager1Window_.routerBase + '/prohibit-access'
    return Promise.reject(data)
  }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (result.code === 200) {
          ...
          reject(new Error('refresh token failed: ' + result.msg))
        }
      } catch (error) {
        toLoginOnly(t('i18n_72d46ec2cf'), t('i18n_edc1185b8e') + error + t('i18n_d3fb6a7c83'))
        reject(error)
      } finally {
        // 无论成功/失败/异常都必须复位，否则排队请求将永久空转
        isRefreshing = false
      }
    })
  } else {

<template>
  ...
  >
    <n-spin v-if="loading" :tip="$t('i18n_6a8c30bd06')" :spinning="loading">
      <n-skeleton />
    </n-spin>
    <template v-else>
        ...
        <n-space class="tool-bar-end">
          <div>
            {{ $t('i18n_08902526f1')
              ...
              style="width: 150px"
              :options="themeList.map((item) => ({ label: item.name, value: item.theme }))"
          ...
          <div>
            {{ $t('i18n_117a9cbc8d')
              ...
              style="width: 150px"
              :options="modeList.map((item) => ({ label: item.name, value: item.mode }))"
              @update:value="handleSelectMode"
            />
          </div>

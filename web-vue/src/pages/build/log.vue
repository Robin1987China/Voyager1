      ...
      <n-space>
        <span v-if="status">
          ...
          <n-tooltip>
            <template #trigger>
              <span class="tw">
                <n-tag :color="statusColor[status]" style="margin-right: 0">
        ...
        <span>
          {{ $t('i18n_31aaaaa6ec') }}
          <n-tag>{{ temp && temp.buildId }}</n-tag>
        </span>
        <n-button type="primary" :disabled="!logId" size="small" @click="handleDownload">
          <DownloadOutlined />
          {{ $t('i18n_f26ef91424') }}
        </n-button>
        <n-button type="primary" :disabled="!logId" size="small" @click="() => (environmentVisible = true)">
...
<script>
import { DownloadOutlined, EyeInvisibleOutlined, UnorderedListOutlined } from '@ant-design/icons-vue'

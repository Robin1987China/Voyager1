...
<n-data-table
    ...
    >
      <n-form ref="editForm" :rules="rules" :model="temp">
        <n-form-item :label="$t('i18n_66e9ea5488')" path="name">
          <n-input v-model:value="temp.name" :max-length="50" :placeholder="$t('i18n_7c9bb61536')" />
        </n-form-item>
        <n-form-item :label="$t('i18n_83f25dbaa0')" required>
          <n-space direction="vertical" style="width: 100%">
            <n-grid v-for="(item, index) in temp.projectList" :key="index">
              <n-grid-item :span="11">
                ...
                <n-select
                  ...
                  :options="nodeList.map((nodeItem) => ({ label: nodeItem.name, value: nodeItem.id }))"
                  @update:value="
                    () => {
                      temp = {
                        ...temp,
              ...
              <n-grid-item :span="11">
                ...
                <n-select
                  ...
                  :options="
                    nodeProjectList[item.nodeId].projects.map((project) => ({
                      label: project.name,
              ...
              <n-grid-item :span="2">
                <n-button type="primary" danger @click="() => temp.projectList.splice(index, 1)"
                  ><DeleteOutlined
                /></n-button>
              </n-grid-item>
            </n-grid>

            <n-button type="primary" @click="() => temp.projectList.push({})">{{ $t('i18n_66ab5e9f24') }}</n-button>
          </n-space>
        </n-form-item>
      </n-form>
    </CustomModal>

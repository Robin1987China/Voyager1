<template>
  <div>
    <n-form ref="ruleForm" :model="pruneForm" :rules="rules">
      <n-form-item>
        <n-alert :title="$t('i18n_bc2c23b5d2')" banner />
      </n-form-item>
      <n-form-item
      ...
      >
        <n-select
          v-model:value="pruneForm.pruneType"
          :placeholder="$t('i18n_ad209825b5')"
          :options="Object.entries(pruneTypes).map(([key, item]) => ({ label: item.name, value: key }))"
        />
      </n-form-item>
      <n-form-item
      ...
      >
        <n-switch
          v-model:value="pruneForm.dangling"
          :checked-label="$t('i18n_a09375d96c')"
          :unchecked-label="$t('i18n_643f39d45f')"
        />
      </n-form-item>
      <n-form-item
      ...
      >
        <template #help
          ><n-tag color="#f50"> {{ $t('i18n_1775ff0f26') }},{{ $t('i18n_143d8d3de5') }}</n-tag>
        </template>
        <n-tooltip>
          <template #trigger>
      ...
      <n-form-item>
        <n-button type="primary" :loading="loading" @click="onPruneSubmit">
          {{ $t('i18n_38cf16f220') }}
        </n-button>
      </n-form-item>
    </n-form>

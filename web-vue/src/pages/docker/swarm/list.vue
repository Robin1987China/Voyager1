<template>
  <div>
    <template v-if="useSuggestions">
      <n-result :title="$t('i18n_55e690333a')" :sub-title="$t('i18n_9878af9db5')">
        <template #extra>
          ...
          <n-space>
            <template v-if="record.machineDocker">
              ...
              <n-button
                ...
                >{{ $t('i18n_3bf3c0a8d6') }}</n-button
              >
            </template>
            <template v-else>
              <n-button size="small" :disabled="true" type="primary">{{ $t('i18n_47d68cd0f4') }}</n-button>
              <n-button size="small" :disabled="true" type="primary">{{ $t('i18n_3bf3c0a8d6') }}</n-button>
            </template>
    ...
    >
      <n-form ref="editForm" :rules="rules" :model="temp">
        <n-form-item :label="$t('i18n_c3f28b34bb')" path="name">
          <n-input v-model:value="temp.name" :placeholder="$t('i18n_a51cd0898f')" />
        </n-form-item>

        <n-form-item :label="$t('i18n_14d342362f')" path="tag"
          ><n-input v-model:value="temp.tag" :placeholder="$t('i18n_77017a3140')" />
        </n-form-item>
      </n-form>
    </CustomModal>
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          key: ['machineDocker', 'swarmCreatedAt'],
          sorter: true,
          ellipsis: true,
          render: (row) => parseTime(row['modifyTimeMillis']),
          width: '170px'
        },
        {
          ...
          key: ['machineDocker', 'swarmUpdatedAt'],
          sorter: true,
          ellipsis: true,
          render: (row) => parseTime(row['modifyTimeMillis']),
          width: '170px'

  ...
  <div>
    <n-form :model="temp">
      <n-form-item :label="$t('i18n_ce23a42b47')" path="name">
        <n-input :placeholder="$t('i18n_5f4c724e61')" :disabled="true" :value="temp.taskData && temp.taskData.name" />
      </n-form-item>

      <n-form-item :label="$t('i18n_f98994f7ec')" path="taskType">
        <n-radio-group :value="temp.taskData && temp.taskData.taskType" :disabled="true">
          <n-radio :value="0"> SSH </n-radio>
          <n-radio :value="1"> {{ $t('i18n_3bf3c0a8d6') }} </n-radio>
        </n-radio-group>
      </n-form-item>

      <n-form-item path="releasePath" :label="$t('i18n_dbb2df00cf')">
        <n-input
      ...
      <n-form-item path="releasePath" :label="$t('i18n_3fea7ca76c')" :help="temp.taskData && temp.taskData.statusMsg">
        {{ statusMap[temp.taskData && temp.taskData.status] || $t('i18n_1622dc9b6b') }}
      </n-form-item>

      <n-form-item :label="$t('i18n_c84ddfe8a6')">
        <n-tabs :active-key="activeKey" @change="tabCallback">
          <n-tab-pane v-for="item in temp.taskList" :key="item.id">
            <template #tab>

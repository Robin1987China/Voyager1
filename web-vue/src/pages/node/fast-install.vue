<template>
  <div>
    <div v-if="fastInstallInfo">
      <n-collapse v-model:value="fastInstallActiveKey">
        <n-collapse-item key="1" :header="$t('i18n_947d983961')">
          <n-alert title="" type="warning" show-icon>
        ...
        <n-collapse-item key="2" :header="$t('i18n_70b5b45591')">
          <n-tabs :default-active-key="0">
            <n-tab-pane v-for="(item, index) in fastInstallInfo.shUrls" :key="index" :tab="item.name">
              <div>
                <n-alert type="info" :title="`${$t('i18n_ccb91317c5')}`">
                  <template #description>
                    <n-p style="margin-bottom: 0">
                      <span>{{ item.allText }} </span>
                      <copy-text :text="item.allText" />
                    </n-p>
                  </template>
                </n-alert>
              </div>
        ...
        <n-collapse-item key="3" :header="$t('i18n_dab864ab72')">
          <n-alert type="info" :title="`${$t('i18n_ccb91317c5')}(${$t('i18n_a8920fbfad')})`">
            <template #description>
              <n-p style="margin-bottom: 0">
                <span>{{ fastInstallInfo.bindCommand }} </span>
                <copy-text :text="fastInstallInfo.bindCommand" />
              </n-p>
            </template>
          </n-alert>
        </n-collapse-item>
        <n-collapse-item key="4" :header="$t('i18n_adaf94c06b')">
          ...
          <n-alert
            ...
            :title="`${$t('i18n_ac00774608')} ${index + 1} ${$t('i18n_dbc0b66ca4')}`"
            ...
            <template #description>
              <n-space direction="vertical" style="width: 100%">
                <div v-if="item.type === 'canUseIpEmpty'">
                  <n-tag color="orange">{{ $t('i18n_5d803afb8d') }}</n-tag>
                </div>
                <div v-if="item.type === 'multiIp'">
                  <n-tag color="green">{{ $t('i18n_d9569a5d3b') }}</n-tag>
                </div>
                <div v-if="item.type === 'exists'">
                  <n-tag color="orange">{{ $t('i18n_9bd451c4e9') }}</n-tag>
                </div>
                <div v-if="item.type === 'success'">
                  <n-tag color="orange">{{ $t('i18n_1974fe5349') }}</n-tag>
                </div>
                <div>
                  {{ $t('i18n_8cd628f495')
                  }}<n-tag v-for="(itemIp, indexIp) in item.allIp" :key="indexIp">{{ itemIp }}:{{ item.port }}</n-tag>
                </div>
                <div v-if="item.type === 'multiIp'">
                ...
                <div v-if="item.type === 'success' || item.type === 'exists'">
                  {{ $t('i18n_69c743de70') }}:
                  <n-tag v-for="(itemIp, indexIp) in item.canUseIp" :key="indexIp">{{ itemIp }}:{{ item.port }}</n-tag>
                </div>
              </n-space>
            </template>
          </n-alert>

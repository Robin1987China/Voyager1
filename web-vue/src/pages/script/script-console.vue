  ...
  <div>
    <div>
      <log-view1 ref="logView" height="calc(100vh - 140px)">
        <template #before>
          <n-space>
            <n-button size="small" :loading="btnLoading" :disabled="scriptStatus !== 0" type="primary" @click="start">{{
              $t('i18n_1a6aa24e76')
            }}</n-button>
            <n-button size="small" :loading="btnLoading" :disabled="scriptStatus !== 1" type="primary" @click="stop">{{
              $t('i18n_095e938e2a')
            }}</n-button>
          </n-space>
        </template>
    ...
    <CustomModal
      ...
      @ok="startExecution"
    >
      <n-form ref="ruleForm" :model="temp">
        <!-- <n-form-item label="执行参数" path="args">
            <n-input v-model="temp.args" placeholder="执行参数,没有参数可以不填写" />
          </n-form-item> -->
        <n-form-item :label="$t('i18n_abba4775e1')" :help="`${commandParams.length ? $t('i18n_916cde39c4') : ''}`">
          <n-space direction="vertical" style="width: 100%">
            ...
            <n-button type="primary" size="small" @click="() => commandParams.push({})">{{
              $t('i18n_4c0eead6ff')
            }}</n-button>
          </n-space>
        </n-form-item>

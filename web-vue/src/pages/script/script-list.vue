  ...
  <div>
        ...
        <template v-else-if="column.dataIndex === 'operation'">
          <n-space>
            <template v-if="mode === 'manage'">
              ...
              <n-dropdown
                :options="[
                  { label: $t('i18n_4696724ed3'), key: '0', props: { onClick: () => handleTrigger(record) } },
                  { label: $t('i18n_2f4aaddde3'), key: '1', props: { onClick: () => handleDelete(record) } },
                  {
                    label: $t('i18n_663393986e'),
                    key: '2',
                    disabled: !record.nodeIds,
                    props: { onClick: () => handleUnbind(record) }
                  }
                ]"
              >
    ...
    >
      <n-form ref="editScriptForm" :rules="rules" :model="temp">
        <n-form-item v-if="temp.id" label="ScriptId" path="id">
          <n-input v-model:value="temp.id" disabled read-only />
        </n-form-item>
        ...
        <n-form-item v-if="temp.prohibitSync" :label="$t('i18n_b499798ec5')">
          <template #help>{{ $t('i18n_06986031a7') }}</template>
          <n-tag v-for="(item, index) in temp.nodeList" :key="index"
            >{{ $t('i18n_5d83794cfa') }}{{ item.nodeName }} {{ $t('i18n_4d85c37f0d') }}{{ item.workspaceName }}</n-tag
          >
        </n-form-item>
        <n-form-item v-else>
          <template #label>
    ...
    >
      ...
      import { h } from 'vue' import { NIcon } from 'naive-ui'
    </CustomDrawer>
    <!-- 同步到其他工作空间 -->
    <CustomModal
    ...
    <!-- <div style="padding-top: 50px" v-if="mode === 'choose'">
      ...
      >
        <n-space>
          ...
          <n-button type="primary" @click="handerConfirm"> {{$t('i18n_e83a256e4f')}} </n-button>
        </n-space>
      </div>
    </div> -->
  </div>
...
<script>
import { DownOutlined, MinusCircleOutlined, QuestionCircleOutlined } from '@ant-design/icons-vue'

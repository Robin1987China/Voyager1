<template>
  <n-tabs default-active-key="1" @change="tabChange">
    <n-tab-pane name="1">
      <template #tab>
        <span>
          <SettingOutlined />
        ...
        <n-form-item>
          ...
          >
            <template #tool_before>
              <n-alert v-if="temp.file" show-icon :title="`${$t('i18n_37c1eb9b23')}:${temp.file}`" />
            </template>
          </code-editor>
        </n-form-item>
        <n-form-item>
          <n-space>
            <n-button type="primary" class="btn" @click="onSubmit(false)">{{ $t('i18n_be5fbbe34c') }}</n-button>
            <n-button type="primary" danger class="btn" @click="onSubmit(true)">{{ $t('i18n_6aab88d6a3') }}</n-button>
          </n-space>
        </n-form-item>
      </n-form>
    </n-tab-pane>
    <n-tab-pane name="2" class="ip-config-panel">
      <template #tab>
        <span>
          <LockOutlined />
          {{ $t('i18n_decef97c7c') }}
      ...
      <n-alert :title="`${$t('i18n_22cf31df5d')}${ipTemp.ip}`" type="success" />
      <n-alert :title="$t('i18n_578adf7a12')" style="margin-top: 10px" banner />
      <n-alert :title="$t('i18n_49645e398b')" style="margin-top: 10px" banner />
      <n-form ref="editIpConfigForm" style="margin-top: 10px" :model="ipTemp">
        <n-form-item path="prohibited">
          <template #label>
        ...
        <n-form-item class="ip-config-button">
          <n-button type="primary" class="btn" @click="onSubmitIp()">{{ $t('i18n_be5fbbe34c') }}</n-button>
        </n-form-item>
      </n-form>
    </n-tab-pane>
    ...
    <n-tab-pane name="6">
      ...
      <n-alert :title="`${$t('i18n_c9b79a2b4f')}`" style="margin-top: 10px; margin-bottom: 20px" banner />
      <n-grid justify="center" type="flex">
        <n-form ref="editProxyForm" :model="proxyConfigData">
          <n-grid v-for="(item, index) in proxyConfigData.globalProxy" :key="index">
            <n-space>
              <n-form-item :label="$t('i18n_3ac34faf6d')" path="pattern">
                <n-input
              ...
              <n-form-item :label="$t('i18n_fc954d25ec')">
                <n-input v-model:value="item.proxyAddress" style="width: 30vw" :placeholder="$t('i18n_dcf14deb0e')">
                  <template #prefix>
                    <n-select
                      v-model:value="item.proxyType"
                      style="width: 100px"
                      :options="[
          ...
          <n-form-item>
            <n-space>
              <n-button
                ...
                "
                >{{ $t('i18n_66ab5e9f24') }}</n-button
              >
              <n-button type="primary" @click="saveProxyConfigHannder">{{ $t('i18n_be5fbbe34c') }}</n-button>
            </n-space>
          </n-form-item>
        </n-form>
      </n-grid>
...
<script>
import { ApiOutlined, CheckCircleFilled, LockOutlined, SettingOutlined, StopFilled } from '@ant-design/icons-vue'

import { ACTIVE_MENU_KEY, ACTIVE_TAB_KEY, TAB_LIST_KEY, getStorageJson } from '@/utils/const'
...
export const useAllMenuStore = defineStore('menu2', {
  state: (): any => ({
    ...
    normal_tabList: getStorageJson(TAB_LIST_KEY, []),
    ...
    management_tabList: getStorageJson(MANAGEMENT_TAB_LIST_KEY, []),
    ...
    removeTab(mode: any, key: any) {
      return new Promise((resolve) => {
        ...
        if (this[mode + '_activeTabKey'] === key) {
          // 激活相邻 tab（优先原位置后一个，没有则前一个）
          const tempTab = tabList[Math.min(index, tabList.length - 1)]
          // 如果还是原来激活的 tab 就不用更新
          if (tempTab && this[mode + '_activeTabKey'] !== tempTab.key) {

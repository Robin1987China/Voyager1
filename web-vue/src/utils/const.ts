...
// 常量池
export const USER_NAME_KEY = 'Voyager1-UserName'

export const TOKEN_KEY = 'Voyager1-Token'

export const LONG_TERM_TOKEN = 'Voyager1-Long-Term-Token'

export const USER_INFO_KEY = 'Voyager1-User'

export const MENU_KEY = 'Voyager1-Menus'

export const TOKEN_HEADER_KEY = 'Authorization'

export const ACTIVE_TAB_KEY = 'Voyager1-ActiveTab'

export const TAB_LIST_KEY = 'Voyager1-TabList'

export const ACTIVE_MENU_KEY = 'Voyager1-ActiveMenu'

export const MANAGEMENT_ACTIVE_TAB_KEY = 'Voyager1-management-ActiveTab'

export const MANAGEMENT_TAB_LIST_KEY = 'Voyager1-management-TabList'

export const MANAGEMENT_ACTIVE_MENU_KEY = 'Voyager1-management-ActiveMenu'

// export const GUIDE_FLAG_KEY = "Voyager1-GuideFlag";

// export const GUIDE_HOME_USED_KEY = "Voyager1-Home-Guide-Used";

// export const GUIDE_NODE_USED_KEY = "Voyager1-Node-Guide-Used";
...
/**
 * 安全读取 localStorage 中的 JSON 数据，缓存损坏时回退默认值（避免启动白屏）
 */
...
export function parseTime(time: any, cFormat = 'YYYY-MM-DD HH:mm:ss') {
  ...
  // 纯数字（或数字字符串）才按时间戳处理；'YYYY-MM-DD' 等日期串直接交给 dayjs 解析
  const isNumeric = typeof time === 'number' || (typeof time === 'string' && /^\d+$/.test(time.trim()))
  if (isNumeric) {
    time = Number(time)
  }
  ...
  if (typeof time === 'object') {
    ...
    // 10 位纯数字按 unix 秒处理
    if (isNumeric && ('' + time).length === 10) time = time * 1000
    date = new Date(time)
  }
...
export function formatUnits(value: any, base: number, unitArr: string[], defaultValue = '-') {
  ...
  // 0<value<1 时 index=-1、超大值 index 越界，均需收敛到合法区间
  index = Math.min(Math.max(index, 0), unitArr.length - 1)
...
export function pageBuildInfo() {
  const htmlVersion = (document.head.querySelector('[name~=voyager1-version][content]') as any)?.content
  ...
  return {
    ...
    t: buildTime,
    e: buildEnv,
    df: (document.title || '').toLowerCase().includes('voyager1'),
    t2: Date.now()
  }
}
